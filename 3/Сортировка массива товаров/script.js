...
function sortByFeedbacks(arr) {   // сортировать по отзывам
    //const temp = arr.slice();     // Этот вориант не подходит
    const temp = JSON.parse(JSON.stringify(arr)); // Тут уже получиться создать независимою функцию

    temp.forEach(item => {
        item.ratingRevievs = +item.ratingRevievs.replace(/\D/g, ''); // "156 отзывов" -> "156" -> 156
    });
...
function sortByPrice(arr) {  // сортировать по цене
    const temp = JSON.parse(JSON.stringify(arr)); // Тут уже получиться создать независимою функцию
